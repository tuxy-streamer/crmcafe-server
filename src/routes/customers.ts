...
export async function registerCustomerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/customers", async (req) => {
    const b = req.body as Partial<{
      assigned_user_id: number; name: string; email: string | null; phone_number: string | null; company_name: string | null; address: string | null;
    }>;
  ...
  app.delete("/customers/:id", async (req) => {
    const id = Number((req.params as { id: string }).id);
    const rows = await sql`DELETE FROM customers WHERE customer_id = ${id} RETURNING *`;
