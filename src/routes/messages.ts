...
export async function registerMessageRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/messages", async (req) => {
    const b = req.body as Partial<{
      customer_id: number; user_id: number | null; channel: string; message_body: string; status: string | null; timestamp: string | null;
    }>;
  ...
  app.delete("/messages/:id", async (req) => {
    const id = Number((req.params as { id: string }).id);
    const rows = await sql`DELETE FROM messages WHERE message_id = ${id} RETURNING *`;
